...
void main() {
  ...
  group('RestaurantListProvider Test', () {
    test('should return ResultStateInitial when provider is initialized.', () {
      expect(provider.state, isA<ResultStateInitial<RestaurantListResponse>>());
    });

    test(
      'should return ResultStateSuccess containing restaurant list when API successfully fetch data',
      () async {
        ...
        expect(
          provider.state,
          isA<ResultStateSuccess<RestaurantListResponse>>(),
        );
      },
    );

    test(
      'should return ResultStateError when API fails to fetch data.',
      () async {
        when(
          () => mockApiService.getRestaurantList(),
        ).thenThrow(Exception('Failed to load internet'));
